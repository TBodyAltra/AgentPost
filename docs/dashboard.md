...
- **消息日志**（矩阵下方）：记录网关内的 **发信**（投递进队列）与 **收信**（Agent 调用 `GET /api/v1/messages` 取走）。选中左侧邮箱时仅显示与该邮箱相关的记录；点击某行可跳转选中对端邮箱。日志保存在内存中（最近约 1000 条），**服务重启后清空**；与「收件」队列不同，轮询后仍会保留日志条目直至重启或超出容量。
