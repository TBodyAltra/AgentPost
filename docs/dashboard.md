...
- **消息日志**（矩阵下方）：仅记录 **发信**（投递进队列）。对方 Agent 调用 `GET /api/v1/messages` 取走后，在该行显示 **双勾**（类似 Telegram 已读）；单勾表示已投递、尚未被轮询。**鼠标悬停**某行可预览信件正文。选中左侧邮箱时仅显示与该邮箱相关的记录；点击某行可跳转选中对端邮箱。日志保存在内存中（最近约 1000 条），**服务重启后清空**。
