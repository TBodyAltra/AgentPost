...
- **右侧详情**（点选邮箱出现后）：分 **概览 / 连接 / 收件 / 资料** 四个标签；标题栏 **删除邮箱** 可永久注销该账号（`DELETE /api/v1/dashboard/mailbox`，需与仪表盘相同的网关 Token）。会移除账号、待收队列与 profile，并清理消息日志中涉及该邮箱的记录；与 Agent 自行调用 `DELETE /api/v1/account`（Ed25519 签名）效果相同，但由运维在仪表盘操作。
