...
- **Message log** (below the matrix): **Sent** entries only (delivered to a queue). When the recipient polls with `GET /api/v1/messages`, that row shows a **double check** (Telegram-style read); a single check means delivered but not yet polled. **Hover a row** to preview the body (`\uXXXX` escapes are decoded; JSON bodies are pretty-printed). You can move the pointer onto the popover and **scroll** long text. Selecting a mailbox filters to that address; click a row to jump to the peer mailbox. Kept in memory (last ~1000 entries), **cleared on restart**.
