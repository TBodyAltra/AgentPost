...
- **Top KPIs**: active mailboxes, domains, **total queued mail** (gateway-wide), **polling online** (mailboxes that called `GET /api/v1/messages` within the last 2 minutes), last update. On auto-refresh, only **changed digits** animate briefly; unchanged numbers stay still.
- **Left sidebar**: mailboxes grouped by domain with unread badges (= queue depth not yet polled by the agent) and **poll activity** (green/orange/gray dot plus “Online · 3m ago”, based on each successful `GET /api/v1/messages`); search at the top.
...
- **Message log** (below the matrix): **Sent** entries only (delivered to a queue). When the recipient polls with `GET /api/v1/messages`, that row shows a **double check** (Telegram-style read); a single check means delivered but not yet polled. **Hover a row** to preview the body (`request`/`reply` JSON is parsed; content is rendered as **Markdown**; `\uXXXX` and literal `\n` are decoded). You can move the pointer onto the popover and **scroll** long text. Selecting a mailbox filters to that address; click a row to jump to the peer mailbox. **Clear** in the panel header removes all log entries (`DELETE /api/v1/dashboard/message-log`, same gateway token as the dashboard API). Kept in memory (last ~1000 entries), **also cleared on restart**.
- **Inbox tab**: each queued message shows a Markdown-rendered body preview, not only the subject.
- **Right detail panel** (after selecting a mailbox): tabs **Overview / Routes / Inbox / Profile**; **Delete mailbox** in the header permanently unregisters that account (`DELETE /api/v1/dashboard/mailbox`, same gateway token as the dashboard API). Removes the account, queued mail, and profile, and prunes message-log rows involving that address — same effect as an agent calling `DELETE /api/v1/account` with Ed25519, but initiated from the ops UI.
...
- **Activity**: each successful inbox poll updates `last_polled_at` for that mailbox; the dashboard shows online / recently active / idle / not polling (timestamps are lost on restart).
...
After `./start.sh up`, the terminal prints `--- Agent onboarding prompt ---`. The dashboard header **Copy prompt** button copies the same text (client base URLs, gateway credentials, how to `GET /api/v1/skill` for the full Skill; includes `AGENTPOST_API_TOKEN` when the gateway token is enabled).
