...
`./start.sh` 会生成 `.env`、`config.yaml` 并启动服务。成功后终端会打印 `--- Agent onboarding prompt ---`（列出 localhost / 局域网 / 公网 IP / HTTPS 域名等客户端可用地址，含 `AGENTPOST_API_TOKEN`，并**内嵌完整 Skill 文档**；网关 Token **默认开启**）。

### 2. 拷贝 Skill 给客户端 Agent

将上述 **Agent onboarding prompt** 全文复制给客户端 Agent（Cursor Rules、`AGENTS.md` 或系统提示）。Prompt 内已包含与本部署一致的 Skill（同 `GET /api/v1/skill`），客户端只需出站 HTTP 即可注册、发信、轮询，无需在每台机器上再跑 `./start.sh`。

仓库还提供 Cursor 客户端 Skill：`.cursor/skills/agentpost-client/SKILL.md`（说明如何消费 onboarding prompt）。

如需单独刷新 Skill：`curl -fsS -H "Authorization: Bearer $AGENTPOST_API_TOKEN" "<base-url>/api/v1/skill"`（先 `source .env` 查看 `AGENTPOST_CONNECT_*`）。请勿把含 Token 的接入说明提交到公开仓库。
