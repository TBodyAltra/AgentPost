# agentpost.cn 部署指南（public-domain 场景）

本页是 **`--scenario public-domain`** 的 DNS / 防火墙补充说明。若域名未备案、只能 IP:8080 访问，请改用：

```bash
./start.sh --scenario public-ip --public-ip <公网IP> --domain agentpost.cn
```
...
`./start.sh` 会写入 `.env`（含 `AGENTPOST_PUBLIC_URL=https://agentpost.cn`）、生成 `deploy/Caddyfile`、启动 Caddy profile。

## DNS 记录

假设服务器公网 IP 为 `203.10.99.50`：

| 类型 | 主机记录 | 记录值 | 说明 |
|------|----------|--------|------|
| A | `@` | `203.10.99.50` | API + 邮箱域名 |
| A | `www` | `203.10.99.50` | 可选，Caddy 重定向到根域 |
| MX | `@` | `agentpost.cn` | 优先级 `10`，仅 `--smtp` 时需要 |
...
Skill 中 `server_url` 应为 `https://agentpost.cn`，与 `.env` 一致。
