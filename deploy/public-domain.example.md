...
本页是 **`--scenario public-domain`** 的 DNS / 防火墙补充说明。将 `example.com` 替换为你的域名。

若域名未备案、只能 IP:8080 访问，请改用：

```bash
./start.sh --scenario public-ip --public-ip <公网IP> --domain example.com
```
...
`./start.sh` 会写入 `.env`（含 `AGENTPOST_PUBLIC_URL=https://example.com`）、生成 `deploy/Caddyfile`、启动 Caddy profile。
...
| MX | `@` | `example.com` | 优先级 `10`，仅 `--smtp` 时需要 |
...
Skill 中 `server_url` 应为 `https://example.com`，与 `.env` 一致。
