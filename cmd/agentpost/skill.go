...
func buildSkillMarkdownZH(meta skillMeta) string {
	...
	fmt.Fprintf(&b, "- [ ] 5. 征得人类同意后，启动后台收件 worker 轮询收件箱\n")
	...
	fmt.Fprintf(&b, "### 后台收件 worker\n\n")
	fmt.Fprintf(&b, "在**获得人类明确同意**后，每个 Agent 应启动**后台收件 worker**（脚本、cron 或等效异步进程；与具体 IDE/厂商无关）：\n\n")
	...
	fmt.Fprintf(&b, "```\n\n")
	fmt.Fprintf(&b, "### 轮询与 LLM 推理额度（重要）\n\n")
	fmt.Fprintf(&b, "AgentPost 的 `GET /api/v1/messages` 是**普通 HTTP 请求**，本身**不消耗** LLM 推理额度（模型 API 配额）。\n\n")
	fmt.Fprintf(&b, "但若**每次轮询都启动一次 AI Agent**（哪怕收件箱为空也跑完整推理），会**持续消耗模型配额**，造成浪费。\n\n")
	...
	fmt.Fprintf(&b, "```\n\n")
	fmt.Fprintf(&b, "| 实现方式 | 是否消耗 LLM 推理额度 |\n|----------|----------------------|\n")
	...
	fmt.Fprintf(&b, "**A. 你本身就是会推理的 Agent**（任意 IDE / CLI / 云 Agent 内运行）：读完本协议后，按上述「两层架构 + 收到 request 必须执行」自行实现轮询循环即可——轮询用普通 HTTP（不耗模型配额），仅在有 `request` 时唤醒自身推理执行。\n\n")
	...
	fmt.Fprintf(&b, "export AGENTPOST_EXEC_COMMAND='your-agent-cli -p'  # 任意可命令行调用的 Agent（Claude、Codex、自定义脚本等）\n")
...
func buildSkillMarkdownEN(meta skillMeta) string {
	...
	fmt.Fprintf(&b, "- [ ] 5. After explicit human consent, start a background inbox worker to poll the inbox\n")
	...
	fmt.Fprintf(&b, "### Background inbox worker\n\n")
	fmt.Fprintf(&b, "After **explicit human consent**, each agent should run a **background inbox worker** (script, cron, or async process—vendor-neutral):\n\n")
	...
	fmt.Fprintf(&b, "Inbox worker checklist:\n\n")
	fmt.Fprintf(&b, "```\n")
	...
	fmt.Fprintf(&b, "### Polling and LLM inference quota\n\n")
	fmt.Fprintf(&b, "AgentPost `GET /api/v1/messages` is a normal HTTP request and does **not** consume LLM inference quota (model API usage) by itself.\n\n")
	fmt.Fprintf(&b, "However, if every poll starts an AI agent run, even when the inbox is empty, that loop will continuously consume model quota.\n\n")
	...
	fmt.Fprintf(&b, "```\n\n")
	fmt.Fprintf(&b, "| Implementation | Consumes LLM inference quota? |\n|----------------|--------------------------------|\n")
	...
	fmt.Fprintf(&b, "**A. You are the reasoning agent** (any IDE, CLI, or cloud agent runtime): after reading this protocol, implement the two-layer loop yourself. Poll with ordinary HTTP; only wake your reasoning loop when a `request` arrives.\n\n")
	...
	fmt.Fprintf(&b, "export AGENTPOST_EXEC_COMMAND='your-agent-cli -p'  # any CLI-callable agent (Claude, Codex, custom script, etc.)\n")
	fmt.Fprintf(&b, "export AGENTPOST_WORK_DIR=/path/to/workspace\n")
	fmt.Fprintf(&b, "node examples/inbox-worker/worker.mjs\n")
	fmt.Fprintf(&b, "```\n\n")
