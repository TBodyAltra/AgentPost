...
func buildSkillMarkdownZH(meta skillMeta) string {
	...
	case "http":
		fmt.Fprintf(&b, "本部署为默认 HTTP 网关（`./start.sh up`）。客户端按上表选择可达地址；可选 HTTPS 域名见 **域名 (HTTPS)** 行。\n\n")
	}
...
func buildSkillMarkdownEN(meta skillMeta) string {
	...
	case "http":
		fmt.Fprintf(&b, "This deployment is the default HTTP gateway (`./start.sh up`). Clients pick a reachable base URL from the table above; optional HTTPS domain is listed when configured.\n\n")
	}
